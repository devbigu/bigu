...
type BrandLoaderProps = { label?: string; fullScreen?: boolean };

export function BrandLoader({ label = "Loading...", fullScreen = false }: BrandLoaderProps) {
  const loader = (
    <div className={styles.loader} role="status" aria-label={label}>
      <div className={styles.box}><div className={styles.logo}>
      ...
      <div className={styles.box} /><div className={styles.box} /><div className={styles.box} /><div className={styles.box} />
      <span className="sr-only">{label}</span>
    </div>
  );
  return fullScreen ? <main className="flex min-h-[60vh] w-full items-center justify-center px-4">{loader}</main> : loader;
}
